...
\item 在进行实验过程中，PKID中的特征只使用了13个（原来计划选用22个）。原因是由于图像质量原因，特征提取效果不是特别好，使一些特征对分类准确率的提升没有太大贡献。另外，还有两个特征我没有找到计算方法，但我感觉对实验结果提高不会有太大影响。
\item 实验中提取的特征不如PkID系统中的特征好（有一部分原因是实验中提取的特征是对浮游动物图像提取的，而浮游动物图像质量较低。PkID系统中的基本特征都是从ZooProcess中得到的。），其分类的准确率也比PkID系统中的特征低2到3个百分点。
\item 为了将单个的特征值和特征向量融合，因此进行了特征融合（实验四、五）。目前只采用了20个特征与LBP特征融合，分类的准确率有了一定的提高。
\end{itemize}
