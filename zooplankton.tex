...
HOG\cite{dalal2005histograms}（Histogram of Oriented Gradients，方向梯度直方图）是用于目标检测的特征描述子，它同统计图像局部区域的梯度方向直方图来构成特征。
...
SIFT（Scale Invariant Feature Transform，尺度不变特征变换）是一直检测局部特征的算法，该算法通过求得一幅图像中的特征点及有关尺度和方向的描述子得到特征并进行图像特征点匹配，它由 David Lowe 在 1999 年\cite{lowe1999object}所发表, 2004 年\cite{lowe2004distinctive}完善总结。%SIFT算法提取的特征点具有尺度不变性，也就是说，同一物体在图像上不论尺度大小，都能根据SIFT算法提取到相同的特征点。
...
\subsubsection{SURF(Speed-Up Robust Feature)}
