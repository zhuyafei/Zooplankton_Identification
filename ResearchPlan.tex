...
\begin{description}
    \item[Appendicularia（尾海鞘纲）] 体型像蝌蚪，身体分为躯干和尾两部分。躯干为椭圆形；尾部扁平，比躯干要长。（见维基百科）\footnote{\url{https://zh.wikipedia.org/}}
    
    观察采集的图像发现：长条形，躯干宽度大于尾部宽度（图像中的躯干并不是规则的椭圆），躯干中心处灰度值较深；尾部灰度相对较浅。
    \item[Bubble(气泡)] 非生物。
    
    观察采集的图像发现：圆形，中心处灰度值较大（接近白色），四周灰度值较小。
    \item[Chaetognatha（毛颚动物门）] 全体分为头、躯干和尾3部分。头部略膨大，躯干与头连接处稍微缢缩为颈部，尾部末端尖细。头部两侧各着生一列4～13根镰刀状、几丁质的颚毛。（见维基百科）\footnote{\url{https://zh.wikipedia.org/}}
    ...
    \item[Copepoda(桡脚类)] 体形像泪珠，有大的触角。分为前体部和后体部，前体部较为宽大，后体部较为短小。（见维基百科）\footnote{\url{https://zh.wikipedia.org/}}
    ...
    \item[Multiple（多类）] 多类生物的集合。
    \item[Nonbio] 非生物的集合。
    \item[Pteropoda（Greseis virgula，牙笔帽螺）] 观察采集的图像发现：像象牙，一端粗一端细，灰度相对较深。
\end{description}
...
\subsection{更改去噪方法做的一些实验}

\subsubsection{实验一}
去噪过程：1、将图像imresize为$256 \times 256$。2、二值化，去掉连通区域小于10的噪声。
...
\subsubsection{实验二}
去噪过程：1、将图像imresize为$256 \times 256$。2、二值化，然后进行闭-开-闭运算。3、取最大连通区域。
...
\subsubsection{实验三}
去噪过程：1、将图像imresize为$256 \times 256$。2、二值化，然后进行闭-开-闭运算。
...
\subsubsection{以上做过的实验分析}
\begin{itemize}
\item 先将图像imresize再进行去噪的实验效果要比先去噪再imresize的实验结果要好。
\item 保留最大连通区域虽然可以去除除了目标以外的噪声，但也会造成目标部分丢失，不利于分类准确率的提高。
\item 由于浮游动物不同种类特征不同，在去噪过程中不容易找到一个适用于所有种类图像的去噪方法。
\item 要根据不同的特征描述方法设计不同的去噪方法。
\end{itemize}
