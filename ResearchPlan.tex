...
\begin{description}
    \item[Appendicularia（尾海鞘纲）] 体型像蝌蚪，身体分为躯干和尾两部分。躯干为椭圆形；尾部扁平，比躯干要长。（详见维基百科）
    
    观察采集的图像发现：长条形，躯干宽度大于尾部宽度（图像中的躯干并不是规则的椭圆），躯干中心处灰度值较深；尾部灰度相对较浅。
    \item[Bubble(气泡)] 非生物。
    
    观察采集的图像发现：圆形，中心处灰度值较大（接近白色），四周灰度值较小。
    \item[Chaetognatha（毛颚动物门）] 全体分为头、躯干和尾3部分。头部略膨大，躯干与头连接处稍微缢缩为颈部，尾部末端尖细。头部两侧各着生一列4～13根镰刀状、几丁质的颚毛。（详见维基百科）
    ...
    \item[Copepoda(桡脚类)] 体形像泪珠，有大的触角。分为前体部和后体部，前体部较为宽大，后体部较为短小。（详见维基百科）
