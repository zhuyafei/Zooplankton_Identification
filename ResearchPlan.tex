...
VLFeat\footnote{\url{http://www.vlfeat.org/}}库中有HOG特征的计算。用法为hog = vl\_hog(img, cellSize)，得到的hog特征是一个三维矩阵，其中三维矩阵的列和行分别为图像的列和行被cellSize所分的份数，第三维表示每一份的特征向量。

分类器SVM用的是LibSVM。

将所有图像resize为$256 \times 256$，cellSize取128时对分类结果进行交叉验证（folds = 2, repetitions = 5）所得到的混淆矩阵如图~\ref{fig: HOG-SVM-2-folds-5-repetitions-128-256}。
\begin{figure}[!ht]
\centering
...
将所有图像resize为$256 \times 256$，cellSize取64时对分类结果进行交叉验证（folds = 2, repetitions = 5）所得到的混淆矩阵如图~\ref{fig: HOG-SVM-2-folds-5-repetitions-64-256}。
\begin{figure}[!ht]
\centering
...
将所有图像resize为$256 \times 256$，cellSize取32时对分类结果进行交叉验证（folds = 2, repetitions = 5）所得到的混淆矩阵如图~\ref{fig: HOG-SVM-2-folds-5-repetitions-32-256}。
\begin{figure}[!ht]
\centering
...
将所有图像resize为$256 \times 256$，cellSize取16时对分类结果进行交叉验证（folds = 2, repetitions = 5）所得到的混淆矩阵如图~\ref{fig: HOG-SVM-2-folds-5-repetitions-16-256}。
\begin{figure}[!ht]
\centering
...
将所有图像resize为$256 \times 256$，cellSize取8时对分类结果进行交叉验证（folds = 2, repetitions = 5）所得到的混淆矩阵如图~\ref{fig: HOG-SVM-2-folds-5-repetitions-8-256}。
\begin{figure}[!ht]
\centering
...
将所有图像resize为$64 \times 64$，cellSize取32时对分类结果进行交叉验证（folds = 2, repetitions = 5）所得到的混淆矩阵如图~\ref{fig: HOG-SVM-2-folds-5-repetitions-32-64}。
\begin{figure}[!ht]
\centering
...
将所有图像resize为$64 \times 64$，cellSize取16时对分类结果进行交叉验证（folds = 2, repetitions = 5）所得到的混淆矩阵如图~\ref{fig: HOG-SVM-2-folds-5-repetitions-16-64}。
\begin{figure}[!ht]
\centering
...
将所有图像resize为$64 \times 64$，cellSize取8时对分类结果进行交叉验证（folds = 2, repetitions = 5）所得到的混淆矩阵如图~\ref{fig: HOG-SVM-2-folds-5-repetitions-8-64}。
\begin{figure}[!ht]
\centering
...
将所有图像resize为$64 \times 64$，cellSize取4时对分类结果进行交叉验证（folds = 2, repetitions = 5）所得到的混淆矩阵如图~\ref{fig: HOG-SVM-2-folds-5-repetitions-4-64}。
\begin{figure}[!ht]
\centering
...
以上的实验分为C和Matlab两个版本，两者跑出来的结果差异很大，总体来说，Matlab版的代码跑出来的结果相对较好。Matlab下都是将LibSVM的参数固定，然后调整vl\_hog的参数并进行评价的结果。可以看到，当imgSize为64，cellSize为16时，交叉验证的结果最佳。
...
将所有图像resize为$256 \times 256$，cellSize取128时对分类结果进行交叉验证（folds = 2, repetitions = 5）所得到的混淆矩阵如图~\ref{fig: LBP-SVM-2-folds-5-repetitions-128-256}。
\begin{figure}[!ht]
\centering
...
将所有图像resize为$256 \times 256$，cellSize取64时对分类结果进行交叉验证（folds = 2, repetitions = 5）所得到的混淆矩阵如图~\ref{fig: LBP-SVM-2-folds-5-repetitions-64-256}。
\begin{figure}[!ht]
\centering
...
将所有图像resize为$256 \times 256$，cellSize取32时对分类结果进行交叉验证（folds = 2, repetitions = 5）所得到的混淆矩阵如图~\ref{fig: LBP-SVM-2-folds-5-repetitions-32-256}。
\begin{figure}[!ht]
\centering
...
将所有图像resize为$256 \times 256$，cellSize取16时对分类结果进行交叉验证（folds = 2, repetitions = 5）所得到的混淆矩阵如图~\ref{fig: LBP-SVM-2-folds-5-repetitions-16-256}。
\begin{figure}[!ht]
\centering
...
将所有图像resize为$64 \times 64$，cellSize取32时对分类结果进行交叉验证（folds = 2, repetitions = 5）所得到的混淆矩阵如图~\ref{fig: LBP-SVM-2-folds-5-repetitions-32-64}。
\begin{figure}[!ht]
\centering
...
将所有图像resize为$64 \times 64$，cellSize取16时对分类结果进行交叉验证（folds = 2, repetitions = 5）所得到的混淆矩阵如图~\ref{fig: LBP-SVM-2-folds-5-repetitions-16-64}。
\begin{figure}[!ht]
\centering
...
将所有图像resize为$64 \times 64$，cellSize取8时对分类结果进行交叉验证（folds = 2, repetitions = 5）所得到的混淆矩阵如图~\ref{fig: LBP-SVM-2-folds-5-repetitions-8-64}。
\begin{figure}[!ht]
\centering
...
将所有图像resize为$64 \times 64$，cellSize取4时对分类结果进行交叉验证（folds = 2, repetitions = 5）所得到的混淆矩阵如图~\ref{fig: LBP-SVM-2-folds-5-repetitions-4-64}。
\begin{figure}[!ht]
\centering
