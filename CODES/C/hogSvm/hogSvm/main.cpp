...
int main(int argc, const char * argv[]) {
    ...
    string trainFeatureVector="./result/Train-Features-HOG.txt";
    ...
    for (int i=0; i<2; i++) {
        ...
        for (int j=0; j<2; j++) {
            ...
            if (j==0) {
                confusionMatrixTrainName<<"./result/Train-CM-HOG-SVM-"<<kernelFunction<<".txt";
                confusionMatrix(labelsTrain, predictTrainResult, classifyNum, confusionMatrixTrainName.str());
            }
            else if (j==1) {
                confusionMatrixTestName<<"./result/Test-CM-HOG-SVM-"<<kernelFunction<<".txt";
