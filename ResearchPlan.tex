...
与行人检测不同，在浮游动物分类问题中，没有复杂的背景，所以不需要用窗口进行滑动，判断哪个窗口中有目标，只需将整幅图像看成一个窗口，判断该窗口中的目标属于哪个类别。在本实验中，将图像resize为$64 \times 64$，以整幅图像作为一个窗口提取HOG特征，其中块（block）选用$16 \times 16$，block滑动的步长选用$8 \times 8$，单元（cell）选用$8 \times 8$。则对于一幅图像，所提取的HOG特征向量维数为$(\dfrac{64-16}{8}+1)\times (\dfrac{64-16}{8}+1) \times 4\times 9=1764$维。每个block中的特征向量是对其中的4个cell按从上到下，从左到右的顺序将9维特征向量连起来得到的。而整个窗口中的特征又是对其中的所有block按从上到下，从左到右的顺序将特征连起来得到的。
...
\includegraphics[width=1.0\linewidth]{图像256*256块128*128步长64*64单元64*64}
}
\subfigure[图像256*256块64*64步长32*32单元32*32]{
\includegraphics[width=1.0\linewidth]{图像256*256块64*64步长32*32单元32*32}
}
\subfigure[图像256*256块32*32步长16*16单元16*16]{
\includegraphics[width=1.0\linewidth]{图像256*256块32*32步长16*16单元16*16}
}
\subfigure[图像256*256块16*16步长8*8单元8*8]{
\includegraphics[width=1.0\linewidth]{图像256*256块16*16步长8*8单元8*8}
}
\label{fig:HOG-CV-CM}
\caption{The experimental results.}
\end{figure}

\subsubsection{HOG+C-SVC RBF}
...
\subsubsection{LBP+C-SVC Linear}
