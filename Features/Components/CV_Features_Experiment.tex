...
\subsubsection{实验一}
\begin{description}
\item[选用特征：] 在\ref{ronghe1}所用特征的基础上采用特征融合方法一融合Gabor特征。
\end{description}
...
\subsubsection{实验二}
\begin{description}
\item[选用特征：] 在\ref{ronghe1}所用特征的基础上采用特征融合方法一融合Fourier描述子。
\end{description}
MATLAB：在该实验得到的分类结果如图\ref{fig:20+LBP+Fourier-Features-MATLAB}，其分类准确率为73.6\%。
\begin{figure}[!ht]
\centering
